...
import { colorField, groupField, numberField, rangeField, segmentedField, staticField, toggleField } from './fields.js';
...
export function createSettingsPanel({ dialog, store, onClose, confirmReset }) {
  ...
  dialog.addEventListener('close', () => {
    store.flush();
    // Lets blocks with live previews notice they are no longer visible.
    sync(store.get());
    onClose?.();
  });
